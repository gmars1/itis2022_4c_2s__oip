...
class VectorTFIdfSearcher(Searcher):
    ...
    для каждого слова в поисковом запросе считаем tfidf (как среднее по всем документам),
    получаем вектор запроса (остальные слова заполняем нулями),
    далее для каждого докомента считаем его вектор,
    ...
    def get_docs(self, query: str) -> List[int]:
        ...
        logger.info(f"scores (doc_id, score): {[(doc_id, round(score.item(), 3)) for doc_id, score in scores]}")
